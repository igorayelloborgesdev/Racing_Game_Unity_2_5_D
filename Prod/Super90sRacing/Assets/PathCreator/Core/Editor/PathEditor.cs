...
namespace PathCreationEditor {
    ...
    public class PathEditor : Editor {
        ...
        const float normalsSpacing = .05f;
        ...
        void DrawBezierPathSceneEditor () {
            ...
            if (Event.current.type == EventType.Repaint) {
                ...
                if (data.showNormals) {
                    ...
                    trackSegmentDTO.trackVertexDTOArray = new List<TrackVertexDTO>();
                    Vector3 init1 = Vector3.zero;
                    for (int i = 0; i < normalLines.Length; i += 2)
                    {
                        trackSegmentDTO.trackVertexDTOArray.Add(new TrackVertexDTO());
                        trackSegmentDTO.trackVertexDTOArray[trackSegmentDTO.trackVertexDTOArray.Count - 1].pointsArray = new List<Vector3>();
                        trackSegmentDTO.trackVertexDTOArray[trackSegmentDTO.trackVertexDTOArray.Count - 1].pointsArray.Add(normalLines[i]);
                        trackSegmentDTO.trackVertexDTOArray[trackSegmentDTO.trackVertexDTOArray.Count - 1].pointsArray.Add(normalLines[i + 1]);
                        init1 = normalLines[i];
                    }
                    string dataAsJson = JsonConvert.SerializeObject(trackSegmentDTO);
                    string filePath = "D:/IGOR/IgorAyelloBorges/PROJETOS/16Racing/Prod/Super90sRacing/Assets/Scenes/TrackEditor/TrackLine_00.json";
